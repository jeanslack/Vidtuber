#!/bin/bash
#
# Description: Build a `Vidtuber-*-x86_64.AppImage` starting
...
#
#   ./Vidtuber*.AppImage --appimage-extract
...
# Set a temporary dir
BUILD_DIR=$(mktemp -d -p "$TEMP_BASE" vidtuber-AppImage-build-XXXXXX)
...
# install vidtuber and its dependencies
if [ -f $REPO_ROOT/dist/vidtuber-*.whl ]; then
    $APP_DIR/AppRun -m pip install $REPO_ROOT/dist/vidtuber-*.whl

else
    $APP_DIR/AppRun -m pip install vidtuber
fi

# remove old appdata on metainfo dir
rm $APP_DIR/usr/share/metainfo/python*.appdata.xml

# copy new appdata file for Vidtuber
cp -f $REPO_ROOT/io.github.jeanslack.vidtuber.appdata.xml \
    $APP_DIR/usr/share/metainfo/io.github.jeanslack.vidtuber.appdata.xml

# copy new AppRun for Vidtuber
...
cp -f $REPO_ROOT/vidtuber/art/io.github.jeanslack.vidtuber.desktop \
    $APP_DIR/usr/share/applications/io.github.jeanslack.vidtuber.desktop
...
export VERSION=$(cat \
    $APP_DIR/opt/python*/lib/python3.9/site-packages/vidtuber-*.dist-info/METADATA \
        | grep "^Version:.*" | cut -d " " -f 2)

# Now, build AppImage using linuxdeploy
./linuxdeploy-x86_64.AppImage --appdir $APP_DIR \
    --icon-file $APP_DIR/opt/python*/share/icons/hicolor/256x256/apps/vidtuber.png \
        --desktop-file $APP_DIR/opt/python*/share/applications/io.github.jeanslack.vidtuber.desktop \
            --output appimage

# move built AppImage back into original CWD
mv Vidtuber*.AppImage "$OLD_CWD/"
